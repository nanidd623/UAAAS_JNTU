...
{
    public class BASFlagDeficiencyFacultyController : BaseController
    {
        //Written By Siva
        ...
        public ActionResult AllCollegesBASFlagNotClearedFaculty(string command)
        {
            ...
            var collegeids = db.jntuh_college_edit_status.Where(e => e.IsCollegeEditable == false && e.academicyearId== PresentYear).Select(e => e.collegeId).ToList();
            collegeids.Add(100);
            collegeids.Add(297);
            collegeids.Add(313);
            //var collegeids = new int[] { 7};
            List<jntuh_college_basreport> basFlagTotal = db.jntuh_college_basreport.Where(e => collegeids.Contains(e.collegeId) && (e.year == 2019 || e.year == 2020)).Select(e => e).ToList();
            var jntuh_college_faculty_registered = db.jntuh_college_faculty_registered.Where(e => collegeids.Contains(e.collegeId)).Select(e => e).ToList();
            var jntuh_college = db.jntuh_college.Where(e => e.isActive == true && collegeids.Contains(e.id)).ToList();
            ...
            int days = Convert.ToInt32(ConfigurationManager.AppSettings["Days"]);
            int months = Convert.ToInt32(ConfigurationManager.AppSettings["Months"]);
            int Secondmonths = Convert.ToInt32(ConfigurationManager.AppSettings["SecondMonths"]);
            int MarchDaysCount = Convert.ToInt32(ConfigurationManager.AppSettings["MarchDays"]);
            int MarchWorkingDaysCount = 0;

            List<CollegeWiseBasFlagFaculty> CollegeWiseBasFlagFaculty = new List<CollegeWiseBasFlagFaculty>();

            foreach (var college in jntuh_college)
            {
                MarchWorkingDaysCount = 0;
                int CollegeWiseFacultyBasCount = 0;
                var Faculty = jntuh_college_faculty_registered.Where(e => e.collegeId == college.id).Select(e => e).ToList();
                foreach (var EachFaculty in Faculty)
                {

                    var BasData = basFlagTotal.Where(e => e.RegistrationNumber.Trim() == EachFaculty.RegistrationNumber.Trim() && e.collegeId == EachFaculty.collegeId && (e.year == 2019 || e.year == 2020)).Select(e => e).ToList();

                    CollegeWiseBasFlagFaculty BasFacultyClass = new CollegeWiseBasFlagFaculty();
                    BasFacultyClass.CollegeCode = college.collegeCode;
                    BasFacultyClass.CollegeName = college.collegeName;
                    BasFacultyClass.Registraionnumber = EachFaculty.RegistrationNumber;
                    string date = BasData.Select(e => e.joiningDate).LastOrDefault().ToString();
                    if (BasData.Count == 0)
                    {
                    ...
                    {
                        BasFacultyClass.Joiningdate = BasData.Select(e => e.joiningDate).LastOrDefault();
                        try
                        {
                             BasFacultyClass.BASJoiningDate = date == null ? null : Convert.ToDateTime(date).ToString("dd-MM-yyyy");
                        }
                    ...
                    foreach (var item in BasData)
                    {
                        ...
                        int? RequiredPresentDays = days;
                        //New Code
                        if (BasFacultyClass.Joiningdate < new DateTime(2019, 06, 01, 00, 00, 00))
                        {
                            if (totalpresentdays < RequiredPresentDays)
                            {
                                if (item.month == "July" && item.year == 2019)
                                {
                                    BasFacultyClass.July = item.month + "(" + totalpresentdays +")";
                                    FacultyBasFlagCount++;

                                }
                                if (item.month == "August" && item.year == 2019)
                                {
                                    BasFacultyClass.August = item.month + "(" + totalpresentdays + ")";
                                    FacultyBasFlagCount++;
                                }
                                else if (item.month == "September" && item.year == 2019)
                                {
                                    BasFacultyClass.September = item.month + "(" + totalpresentdays + ")";
                                    FacultyBasFlagCount++;
                                }
                                else if (item.month == "October" && item.year == 2019)
                                {
                                    BasFacultyClass.October = item.month + "(" + totalpresentdays + ")";
                                    FacultyBasFlagCount++;
                                }
                                else if (item.month == "November" && item.year == 2019)
                                {
                                    BasFacultyClass.November = item.month + "(" + totalpresentdays + ")";
                                    FacultyBasFlagCount++;
                                }
                                else if (item.month == "December" && item.year == 2019)
                                {
                                    BasFacultyClass.December = item.month + "(" + totalpresentdays + ")";
                                    FacultyBasFlagCount++;
                                }
                                else if (item.month == "January" && item.year == 2020)
                                {
                                    BasFacultyClass.January = item.month + "(" + totalpresentdays + ")";
                                    FacultyBasFlagCount++;
                                }
                                else if (item.month == "February" && item.year == 2020)
                                {
                                    BasFacultyClass.February = item.month + "(" + totalpresentdays + ")";
                                    FacultyBasFlagCount++;
                                }
                                else if (item.month == "March" && item.year == 2020)
                                {
                                    BasFacultyClass.March = item.month + "(" + totalpresentdays + ")";
                                    MarchWorkingDaysCount = (int)totalpresentdays;
                                    if (MarchWorkingDaysCount < MarchDaysCount)
                                    {
                                        //FacultyBasFlagCount++;
                                        FalgBas = true;
                                    }
                        ...
                        else if (BasFacultyClass.Joiningdate > new DateTime(2019, 09, 01, 00, 00, 00) && (BasFacultyClass.Joiningdate < new DateTime(2019, 12, 31, 00, 00, 00)))
                        {
                            if (totalpresentdays < RequiredPresentDays)
                            {
                                //if (item.month == "July" && item.year == 2019)
                                ...
                                //    FacultyBasFlagCount++;
                                //}
                                if (item.month == "September" && item.year == 2019)
                                {
                                    BasFacultyClass.September = item.month + "(" + totalpresentdays + ")";
                                    FacultyBasFlagCount++;
                                }
                        ...
                        else if (BasFacultyClass.Joiningdate > new DateTime(2020, 01, 01, 00, 00, 00))
                        {

                            if (item.month == "January" && item.year == 2020)
                            {
                                BasFacultyClass.January = item.month + "(" + totalpresentdays + ")";
                                totalpdays = totalpdays + (int)totalpresentdays;
                            }
                            else if (item.month == "February" && item.year == 2020)
                            {
                                BasFacultyClass.February = item.month + "(" + totalpresentdays + ")";
                    ...
                    var ReqiredMonthCount = BasDataNew - months;
                    //if (FacultyBasFlagCount >= ReqiredMonthCount)
                    if (FacultyBasFlagCount > months && BasFacultyClass.Joiningdate < new DateTime(2019, 06, 01, 00, 00, 00) && FalgBas == false)
                    {
                        FalgBas = true;
                        //if (NotRequiredMonths == 1)
                        //{
                    ...
                    else if (FacultyBasFlagCount > Secondmonths && BasFacultyClass.Joiningdate > new DateTime(2019, 09, 01, 00, 00, 00) && BasFacultyClass.Joiningdate < new DateTime(2019, 12, 31, 00, 00, 00) && FalgBas==false)
                    {
                        FalgBas = true;
                    }
                    else if (BasFacultyClass.Joiningdate > new DateTime(2020, 01, 01, 00, 00, 00) && FalgBas == false)
                    {
                        double average = Math.Ceiling((double)totalpdays / 2);
                        int av = (int)average;
                        if (average < days)
                        {
                            FalgBas = true;
                        }
                        else if (MarchWorkingDaysCount < MarchDaysCount)
                        {
                            FalgBas = true;
                        }
                    }
            ...
            if (command == "Print")
            {
                ...
                Response.ContentType = "application/vnd.ms-excel";
                //return PartialView("~/Views/BASFlagDeficiencyFaculty/_BASFlagFacultyDataExcel.cshtml", CollegeWiseBasFlagFaculty.Where(e => e.Cleared == "NotCleared").ToList());
                return PartialView("~/Views/BASFlagDeficiencyFaculty/_BASFlagFacultyDataExcel.cshtml", CollegeWiseBasFlagFaculty.ToList());
            }
            else
            {
                Response.ClearContent();
