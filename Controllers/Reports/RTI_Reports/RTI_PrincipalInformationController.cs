...
{
    public class RTI_PrincipalInformationController : Controller
    {
        //
        // GET: /RTI_PrincipalInformation/
        ...
        public ActionResult Index(int? collegeid)
        {
           ...
           ///// int userAddressID = db.jntuh_address.Where(a => a.collegeId == collegeid && a.addressTye.Equals("PRINCIPAL")).Select(a => a.id).FirstOrDefault();
            int userPrincipalID = db.jntuh_college_principal_director.Where(e => e.collegeId == collegeid && e.type.Equals("PRINCIPAL")).Select(e => e.id).FirstOrDefault();
            ...
            if (collegeid != null)
            {
                ...
                principalDirector.type = "PRINCIPAL";
        ...
        public ActionResult RTI_AllPrincipalsInformationReport()
        {
            List<jntuh_college> colleges = db.jntuh_college.Where(c => c.isActive == true).Select(c => c).ToList();
            List<PrincipalDirector> PrincipalInformationList = new List<PrincipalDirector>();
            ...
            foreach (var clgInfo in colleges.ToList())
            {
                ...
                { 
                    ...
                    if (jntuh_college_principal != null)
                    {
                        ...
                        PrincipalInformationList.Add(principalDirector);
                    } 
                } 
            }
            int Count = PrincipalInformationList.Count();

            if (Count != 0)
            {
                Response.ClearContent();
                Response.Buffer = true;
                Response.AddHeader("content-disposition", "attachment; filename=RTI_AllPrincipalsInformation.xls");
                Response.ContentType = "application//vnd.ms-excel";
                return PartialView("~/Views/RTI_PrincipalInformation/RTI_AllPrincipalsInformationReport.cshtml", PrincipalInformationList);
            }
            else
            {
                return View("~/Views/RTI_PrincipalInformation/Index.cshtml");
            }
        }
