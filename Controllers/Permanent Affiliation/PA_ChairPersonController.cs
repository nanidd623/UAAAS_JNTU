...
{
    [ErrorHandling]
    public class PA_ChairPersonController : BaseController
    {
        ...
        public ActionResult Create(string collegeId)
        {
            ...
            {
                return RedirectToAction("View", "PA_ChairPerson");
            }
            if (userCollegeID > 0 && userChairPersonID > 0 && (Roles.IsUserInRole("Admin") || Roles.IsUserInRole("DataEntry")))
            {
                return RedirectToAction("Edit", "PA_ChairPerson", new { collegeId = Utilities.EncryptString(userCollegeID.ToString(), WebConfigurationManager.AppSettings["CryptoKey"]).ToString() });
            }
            ...
            bool isPageEditable = db.jntuh_pa_college_screens_assigned.Where(a => a.jntuh_college_screens.ScreenCode.Equals("PCHP") && a.CollegeId == userCollegeID).Select(a => a.IsEditable).FirstOrDefault();
        ...
        public ActionResult Edit(string collegeId)
        {
            ...
            {
                return RedirectToAction("Create", "PA_ChairPerson");
            }
            if (userCollegeID == 0 && (Roles.IsUserInRole("Admin") || Roles.IsUserInRole("DataEntry")))
            {
            ...
            {
                return RedirectToAction("Create", "PA_ChairPerson", new { collegeId = Utilities.EncryptString(userCollegeID.ToString(), WebConfigurationManager.AppSettings["CryptoKey"]).ToString() });
            }
            DateTime todayDate = DateTime.Now.Date;
            int actualYear = db.jntuh_academic_year.Where(a => a.isActive == true && a.isPresentAcademicYear == true).Select(a => a.actualYear).FirstOrDefault();
            int prAy =
